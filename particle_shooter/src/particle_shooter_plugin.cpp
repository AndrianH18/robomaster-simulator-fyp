...
public:
  ...
  void Load(physics::WorldPtr _world, sdf::ElementPtr _sdf)
  {
    ...
    if (_sdf->HasElement("max_vel"))
      this->max_vel = _sdf->Get<double>("max_vel");
    ...
    shoot_sub = nh_.subscribe("/shoot", 10, &ParticleShooterPlugin::updateShoot, this);
  ...
  public: void OnUpdate()
  {
    ...
    else
    {
      try{
            for (int i = 0; i < 4; ++i) {
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/ground_truth/turret_end",
  ...
  void updateShoot(const std_msgs::String::ConstPtr msg)
   {
    ...
    if (this->model_to_update_index_now >= this->modelIDToName_size)
    {
      ...
      UpdateParticles(this->model_to_update_index_now, shooterIndex);
      this->model_to_update_index_now ++;  
   }

  void UpdateParticles(int model_to_update_index, int shooterIndex)
  {
    for (auto model : this->world->Models())
    {
        ...
        {
            this->MoveParticle(model, shooterIndex);
            this->SetForceParticle(model, shooterIndex);
        }
  ...
  void MoveParticle(boost::shared_ptr<gazebo::physics::Model> model, int shooterIndex)
  {
    ...
    if (model_name.find(this->particle_base_name) != std::string::npos)
    {
        ...
        float x_pos = robot_status[shooterIndex].getOrigin().x();
        float y_pos = robot_status[shooterIndex].getOrigin().y();
        float z_pos = robot_status[shooterIndex].getOrigin().z();
        
        model->GetLink("link")->SetLinearVel(ignition::math::Vector3d(0, 0, 0));
        ROS_DEBUG("POSE-RANDOM[X,Y,Z,Roll,Pitch,Yaw=[%f,%f,%f,%f,%f,%f], model=%s", x_pos,y_pos,z_pos,roll_rand,pitch_rand,yaw_rand,model_name.c_str());
        ...
        model->SetWorldPose(
                            ignition::math::Pose3d(
                                ignition::math::Vector3d(x_pos, y_pos, z_pos),
  ...
  void SetForceParticle(boost::shared_ptr<gazebo::physics::Model> model, int shooterIndex)
  {
      
    std::string model_name = model->GetName();
    double roll, pitch, yaw;
    robot_status[shooterIndex].getBasis().getRPY(roll, pitch, yaw);
    float x_axis_vel = this->max_vel*cos(yaw)*cos(pitch);
    float y_axis_vel = this->max_vel*sin(yaw)*cos(pitch);
    float z_axis_vel = this->max_vel*sin(pitch);

    // If the model name contains the substring particle, we consider it a particle
    if (model_name.find(this->particle_base_name) != std::string::npos)
    {
        ROS_WARN("VELOCITY APPLIED[X,Y,Z]=[%f,%f,%f]", x_axis_vel, y_axis_vel, -z_axis_vel);
        //model->GetLink("link")->SetForce(ignition::math::Vector3d(this->x_axis_force, this->y_axis_force, this->z_axis_force));
         model->GetLink("link")->SetLinearVel(ignition::math::Vector3d(x_axis_vel, y_axis_vel, -z_axis_vel));
    }
  ...
  private: ros::Subscriber shoot_sub;
  private: tf::StampedTransform robot_status[4];
  /// \brief World pointer.
