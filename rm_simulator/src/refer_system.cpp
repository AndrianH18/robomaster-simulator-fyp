...
public:
    ...
    nh_(nodehandle)    
    {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            bumper_sub[i] = nh_.subscribe("/jackal"+std::to_string(i)+"/bumper_hit", 10, &ReferSystem::checkArmorHit, this);
            refer_system_msg.robotHealth[i]=2000;
            refer_system_msg.robotShoot[i]=1;
            refer_system_msg.robotMove[i]=1;
        }

        shoot_sub = nh_.subscribe("/shoot", 10, &ReferSystem::updateShoot, this);
    ...
    void checkArmorHit(const std_msgs::String::ConstPtr& msg) {
        char side = (char) msg->data.c_str()[0];
        ...
        if(refer_system_msg.robotHealth[0]<=healthDeduction)
        { //uint format property
            refer_system_msg.robotHealth[0]=0;
            //disable move and shoot
            refer_system_msg.robotMove[0] = 0;
            refer_system_msg.robotShoot[0] = 0;
            ROS_INFO_STREAM("Died!");
        }
        else refer_system_msg.robotHealth[0] -= healthDeduction;
    }
    ...
    void updateShoot(const std_msgs::String::ConstPtr msg){
        ...
        if(refer_system_msg.remain_bullet[selfIndex]>0 && refer_system_msg.robotShoot[selfIndex]==1){
            refer_system_msg.remain_bullet[selfIndex] --;  
        }
    }
    ...
    void game_spin(){
        try{
            for (int i = 0; i < 4; ++i) {
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/ground_truth",
                                         ros::Time(0), robot_status[i]);
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/front_armor",
                                         ros::Time(0), robot_front_armor[i]);
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/left_armor",
                                         ros::Time(0), robot_left_armor[i]);
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/right_armor",
                                         ros::Time(0), robot_right_armor[i]);
                listener.lookupTransform("/map", "/jackal"+std::to_string(i)+"/back_armor",
                                         ros::Time(0), robot_back_armor[i]);                         
            }
        }
...
private:
    ...
    ros::Subscriber                 turret_position_sub[4];
    ros::Subscriber                 bumper_sub[4];      
    ...
    tf::StampedTransform            robot_front_armor[4];
    tf::StampedTransform            robot_right_armor[4];
    tf::StampedTransform            robot_left_armor[4];
    tf::StampedTransform            robot_back_armor[4];
    double                          start_secs=0.;
    float                           turret_position[4] = {0, 0, 0, 0};
